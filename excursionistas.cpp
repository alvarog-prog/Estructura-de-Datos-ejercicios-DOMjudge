...
struct tRes {
    int patrullas;
    int maxExcurisionistas;
};
tRes excursionistas(const BinTree<int>& tree) {

    if (tree.empty()) return{ 0,0 };
    else if (tree.left().empty() && tree.right().empty()) {
        if (tree.root() == 0) return { 0,0 };
        else return { 1,tree.root()};
    }
    else {  
        auto [patrullas_izq, max_ex_izq] = excursionistas(tree.left());
        auto [patrullas_der, max_ex_der] = excursionistas(tree.right());
        tRes res;

        res.patrullas = patrullas_der + patrullas_izq;
        res.maxExcurisionistas = max(max_ex_izq, max_ex_der);
        if (tree.root() != 0) {
            res.maxExcurisionistas += tree.root();
            if (res.patrullas == 0) res.patrullas++;
        }
        return res;
    }
...
void resuelveCaso() {
    ...
    tRes res = excursionistas(tree);
    cout << res.patrullas << " " << res.maxExcurisionistas << endl;
    cout << endl;

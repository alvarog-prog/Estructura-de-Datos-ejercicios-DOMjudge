...
struct tRes {//Lo que voy a devolver
    bool zurdo;
    int nodos;
};

tRes es_zurdo(const BinTree<char>& tree) {
    if (tree.empty()) return{ true,0 };
    else if (tree.left().empty() && tree.right().empty()) return{ true,1 };
    else {
        auto [zurdo_izq, nodos_izq] = es_zurdo(tree.left());
        auto [zurdo_der, nodos_der] = es_zurdo(tree.right());
        tRes res;
        res.nodos = nodos_izq + nodos_der + 1; //tengo que ir actualizando el valor de nodos, aunque solo me interese saber si es zurdo
        res.zurdo = ((zurdo_izq && zurdo_der) && nodos_izq > ((nodos_der + nodos_izq) / 2));
        return res;
    }
}
...
void resuelveCaso() {
    ...
    tRes res = es_zurdo(tree);
    if (res.zurdo) cout << "SI" << endl;
    else  cout << "NO" << endl;
    cout << endl;
}
